...
sudo apt-mark hold amazon-ec2-net-utils
...
### isolated regions can't communicate to awscli.amazonaws.com so installing awscli through 
...
### SOCI Snapshotter ##########################################################
###############################################################################
wget 'https://github.com/awslabs/soci-snapshotter/releases/download/v0.11.1/soci-snapshotter-0.11.1-linux-amd64.tar.gz'
...
if dpkg-query -W -f='${Status}' amazon-ssm-agent 2>/dev/null | grep -q "ok installed"; then
  ...
  if ! [[ -z "${SSM_AGENT_VERSION}" ]]; then
    ...
    # NOTE: The URL must point to a .deb file, not a .rpm file.
    wget -O /tmp/amazon-ssm-agent.deb "https://s3.${BINARY_BUCKET_REGION}.${S3_DOMAIN}/amazon-ssm-${BINARY_BUCKET_REGION}/${SSM_AGENT_VERSION}/linux_${ARCH}/amazon-ssm-agent.deb"
    sudo apt install -y /tmp/amazon-ssm-agent.deb
    rm /tmp/amazon-ssm-agent.deb
  else
    echo "Installing amazon-ssm-agent"
    echo "Attempting install via snap..."
